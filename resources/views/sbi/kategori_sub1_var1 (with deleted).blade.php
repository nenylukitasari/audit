    ...
    @foreach ($kegiatan->kategori as $key => $kategori) 
        ...
        {{-- @if(Auth::user()->role==3)
          @if($kategori->deleted_at == null)
            <th>{{ $kategori->kategori_kegiatan}} USER</th>
          @endif
        @else --}}
        ...
        @if($kategori->deleted_at == null) 
          ...
          <td>
            ...
            <button type="button" class="btn btn-info btn-outline btn-circle btn-xs open-AddBookDialog" data-toggle="modal" data-target="#delete-modal" data-id="{{ $kategori->id }}" data-kode_tabel="{{$kategori->kode_tabel}}"><i class="ti-trash" data-toggle="tooltip" title="Delete Data"></i></button>
...
<script>
  $(document).ready(function (){
    ...
    $(".modal-body #fdelete").attr("action", '/data/delete/'+id+'/'+kode_tabel);
    }); 
});
</script>
<script>
