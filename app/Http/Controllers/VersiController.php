...
class VersiController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // $search = Version::where('status',0)
        //             ->orWhereHas(['jenis_kegiatan', function ($query) use ($key) {         
        //                 $query->where('jenis_kegiatan.jenis_kegiatan','like', "%{$key}%");
        //                 $query->wit(['kegiatan' => function($keg) use ($key){
        //                     //$keg->where('kegiatan.nama_kegiatan','like', "%{$key}%");
        //                     $keg->with(['kategori' => function($kat) use ($key){
        //                        $kat->where('kategori.kategori_kegiatan','like', "%{$key}%");
        //                     }]);
        ...
        // $search = JenisKegiatan::where('version_id', '=', "{$version->id}")
        //                     // ->where('version_id', '=', "{$version->id}")
        //                     ->orWhereHas(['kategori', function ($query) use ($key) {            
        //                         $query->where('kegiatan.nama_kegiatan','like', "%{$key}%");
        //                         $query->orWhere('kategori.kategori_kegiatan','like', "%{$key}%");
        //                     }])
        //                     ->get();
        // $search = JenisKegiatan::where('version_id', '=', "{$version->id}")
        //         ->where
        // $search = $search->whereHas('kegiatan', function($query) use($key) {
        //             $query->where('kegiatan.nama_kegiatan','like', "%{$key}%");
        //             })
        //         ->orWhere('jenis_kegiatan','like', "%{$key}%")
        //         ->get();
        // $search = DB::table('jenis_kegiatan')->where('version_id', '=', "{$version->id}")
        //             ->join('kegiatan','jenis_kegiatan.id', '=', 'kegiatan.jenis_kegiatan_id')
        //             ->join('kategori','kegiatan.id','=','kategori.kegiatan_id')
        //             ->where(function($query) use ($key) {
        //                 $query->where('jenis_kegiatan.jenis_kegiatan','like', "%{$key}%")
        //                     ->orwhere('kegiatan.nama_kegiatan','like', "%{$key}%")
        //                     ->orWhere('kategori.kategori_kegiatan','like', "%{$key}%");
        //              })
        //             ->get();
